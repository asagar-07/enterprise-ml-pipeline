# centralize exception mapping ( ValueError -> 422)
from __future__ import annotations

from typing import Any, Dict, Optional

from fastapi import Request, status
from fastapi.responses import JSONResponse
from fastapi.exceptions import RequestValidationError

from mlPipeline import logger

def _error_payload(error_type: str, message: str, request_id: Optional[str] = None, details: Optional[Dict[str, Any]] = None,) -> Dict[str, Any]:
    payload: Dict[str, Any] = {"error": {"type": error_type, "message": message}}
    if request_id is not None:
        payload["request_id"] = request_id
    if details:
        payload["error"]["details"] = details
    return payload


async def value_error_handler(request: Request, exc: ValueError) -> JSONResponse:
    """
    Map any business/contract validation errors into HTTP 422.

    Raises ValueError for:
      1. missing keys
      2. extra keys
      3.  non-numeric values
      4. incorrect feature length (if ever used)
    """
    logger.info("Validation error: %s", str(exc))
    return JSONResponse(
        status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
        content=_error_payload("ValidationError", str(exc)),
    )


async def unhandled_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    """
    Map unexpected exceptions into HTTP 500.
    Keep the response generic; log details server-side.
    """
    logger.exception("Unhandled exception")
    return JSONResponse(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        content=_error_payload("InternalServerError", "Unexpected server error"),
    )

async def request_validation_error_hanlder(request: Request, exc: RequestValidationError,) -> JSONResponse:
    """
    Mapping FastAPI/Pydantic request validation errors into HTTP 422
    using the same standardized error payload shape.
    """
    logger.info("Request validation error: %s", exc.errors())

    details = {
        "issues": [
            {
                "field": ".".join(str(part) for part in err["loc"]),
                "message": err["msg"],
                "type": err["type"],
            }
            for err in exc.errors()
        ]
    }

    return JSONResponse(
        status_code=status.HTTP_422_UNPROCESSABLE_CONTENT,
        content=_error_payload(
            "RequestValidationError",
            "Invalid request body",
            details=details,
        ),
    )